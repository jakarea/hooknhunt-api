...
export default function CreateProductPage() {
  ...
  useEffect(() => {
    const initQuill = async () => {
      ...
      style.innerHTML = `
          ...
          [data-mantine-color-scheme="dark"] #attributes-editor a,
      ...
      const setupImageInteractions = (quillInstance: any) => {
        ...
        editor.addEventListener('keydown', (e: any) => {
          if ((e.key === 'Delete' || e.key === 'Backspace') && !quillInstance.getSelection()) {
            const selectedImg = editor.querySelector('.richtext_image.selected')
            if (selectedImg) {
              e.preventDefault()
      ...
      if (descriptionContainer && !descriptionQuillRef.current) {
        ...
        quill1.root.addEventListener('dblclick', (e: any) => {
          ...
          if (target && target.tagName === 'IMG' && target.classList.contains('richtext_image')) {
            e.preventDefault()
            // Get the current image blot
            const blot = Quill.find(target)
            if (blot) {
              // Open media library to select replacement image
              openSingleSelect((mediaFile: MediaFile) => {
                // Update the image src
                const img = target as HTMLImageElement
                img.src = mediaFile.url
                // Trigger text-change to update state
                quill1.update()
              })
            }
          }
      ...
      // Initialize Highlights Editor (bullet list only, max 20 items)
        ...
        const updateHighlightsState = () => {
          ...
          if (items.length > 20) {
            ...
            notifications.show({
              ...
              message: t('catalog.productsCreate.maxHighlightsMessage') || 'You can only add up to 20 highlights. Extra items have been removed.',
              color: 'yellow'
            })
            ...
            // Update editor to show only 20 items (prevent recursive event)
      ...
      // Initialize Attributes Editor (bullet list only, max 20 items)
      ...
      if (attributesContainer && !attributesQuillRef.current) {
        ...
        // Update attributes state from editor content
        ...
        // Initialize with existing attributes (if any)
      ...
      // Initialize Bangla Attributes Editor (bullet list only, max 20 items)
    ...
    // Add a small delay to ensure all containers are rendered
    setTimeout(() => {
      initQuill()
    }, 100)
  ...
  const handleUpdateVariant = (id: string, field: keyof ProductVariant, value: any) => {
    setVariants(variants.map(v => {
      ...
      // Mark sellerSku as manually edited when user changes sellerSku directly
  ...
  const handleSaveAsDraft = async (event: React.MouseEvent) => {
    event.preventDefault()
    await submitProduct('draft')
  }

  // Publish product handler
  const handlePublish = async (event: React.MouseEvent) => {
    event.preventDefault()
    ...
    <Box p="md">
      <Stack gap="md">
        <Group justify="space-between">
          ...
          >
            {t('catalog.productsCreate.back') || 'Back to Products'}
          </Button>
        </Group>
        ...
        <form onSubmit={handleSubmit}>
                    ...
                    <Grid>
                      ...
                      <Grid.Col span={6}>
                        <Stack gap="sm">
                          ...
                          <div id="description-bn-editor-container">
                            <Box
                    ...
                    <Grid>
                      <Grid.Col span={6}>
                        <Stack gap="sm">
                          ...
                          <div id="highlights-editor-container">
                            <Box
                      ...
                      <Grid.Col span={6}>
                        <Stack gap="sm">
                          ...
                          <div id="highlights-bn-editor-container">
                            <Box
          ...
          <Group justify="flex-end" gap="sm" mt="md">
            ...
            <Button
              color="green"
              leftSection={isSubmitting ? <IconLoader size={16} /> : <IconCheck size={16} />}
              disabled={isSubmitting}
              loading={isSubmitting}
