...
export default function CreateCustomerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data?.status) {
        notifications.show({
          title: 'Success',
          message: `Customer "${formData.name}" created successfully! Password has been sent to their phone.`,
          color: 'green',
      ...
      notifications.show({
        title: 'Error',
        message: error.response?.data?.message || 'Failed to create customer. Please try again.',
        color: 'red',
      })
    } finally {
      setSubmitting(false)
  ...
  const handleInputChange = (field: keyof FormData, value: string | number | null | boolean) => {
    // Convert null to empty string for consistency (but keep booleans as-is)
      ...
      <Stack>
          ...
          <Group justify="space-between">
            <Box>
              ...
              <Text c="dimmed">Create a new customer account and profile</Text>
            </Box>
            <Button
              component={Link}
              to="/crm/customers"
