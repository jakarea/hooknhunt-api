...
use App\Modules\Affiliate\Models\CategoryAffiliateCommission;
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            if ($request->has('affiliate_id')) {
                $query->where('cac.affiliate_id', $request->input('affiliate_id'));
            }

            // Filter by category
            if ($request->has('category_id')) {
                $query->where('cac.category_id', $request->input('category_id'));
            }

            // Search by category name using direct WHERE clause
            ...
            if ($request->has('is_active')) {
                $query->where('cac.is_active', $request->input('is_active'));
            }

            // Global vs Specific filter
            if ($request->has('type')) {
                if ($request->input('type') === 'global') {
                    $query->whereNull('cac.affiliate_id');
