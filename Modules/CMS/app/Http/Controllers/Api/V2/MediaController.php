...
use App\Modules\CMS\Http\Requests\UploadFileRequest;
...
class MediaController extends Controller
{
    ...
    public function getFiles(Request $request)
    {
        ...
        if (!$user->hasPermissionTo('cms.media.admin')) {
        ...
        if ($request->has('folder_id') && $request->folder_id !== null) {
            ...
            // When no folder_id is specified, show files from all folders
            // But exclude staff documents folder for non-admin users
            if (!$user->hasPermissionTo('cms.media.admin')) {
                $staffFolder = MediaFolder::where('slug', 'staff-documents')->first();
                if ($staffFolder) {
                    $query->where('folder_id', '!=', $staffFolder->id);
                }
            }
            // Don't add whereNull('folder_id') - show all files instead
        }
    ...
    public function bulkDelete(Request $request)
    {
        ...
        if ($staffDocuments->count() > 0 && !$user->hasPermissionTo('cms.media.admin')) {
    ...
    public function bulkMoveFiles(Request $request)
    {
        ...
        if ($request->folder_id) {
            ...
            if ($targetFolder && $targetFolder->slug === 'staff-documents' && !auth()->user()->hasPermissionTo('cms.media.admin')) {
