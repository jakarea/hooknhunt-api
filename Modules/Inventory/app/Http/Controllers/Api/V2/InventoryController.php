...
class InventoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = DB::table('product_variants as pv')
            ...
            ->leftJoin('inventory_batches as ib', 'pv.id', '=', 'ib.product_variant_id')
            ->select(
                'pv.*',
                ...
                'u.name as unit_name',
                DB::raw('COALESCE(SUM(ib.remaining_qty), 0) as current_stock')
            )
            ->groupBy('pv.id', 'p.id', 'p.name', 'p.brand_id', 'p.category_id', 'pv.unit_id', 'b.name', 'c.name', 'u.name')
            ->havingRaw('COALESCE(SUM(ib.remaining_qty), 0) > 0')
