...
class FinancialReportService
{
    ...
    public function generateRatioReport(FinancialReport $report): array
    {
        ...
        if ($config['include_liquidity'] ?? true) {
            ...
            $ratios['Liquidity Ratios'] = [
                ...
                [
                    ...
                    'formula' => '(Current Assets - \App\Modules\Inventory\Models\Inventory) / Current Liabilities',
                ],
            ];
        }
        ...
        if ($config['include_efficiency'] ?? true) {
            ...
            $ratios['Efficiency Ratios'] = [
                [
                    'ratio_name' => '\App\Modules\Inventory\Models\Inventory Turnover',
                    'value' => round($inventoryTurnover, 2),
                    'benchmark' => '6.0',
                    'status' => $inventoryTurnover >= 6 ? 'good' : ($inventoryTurnover >= 3 ? 'fair' : 'poor'),
                    'formula' => 'COGS / Average \App\Modules\Inventory\Models\Inventory',
                ],
                [
    ...
    public function generateCustomReport(FinancialReport $report): array
    {
        ...
        $query = \App\Modules\Finance\Models\ChartOfAccount::query();
    ...
    {
        return \App\Modules\Finance\Models\ChartOfAccount::with(['journalEntries' => function ($q) use ($start, $end) {
    ...
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('type', 'asset')->sum('balance');
    }

    private function getTotalLiabilities($start, $end): float
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('type', 'liability')->sum('balance');
    }

    private function getTotalEquity($start, $end): float
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('type', 'equity')->sum('balance');
    }

    private function getCurrentAssets($start, $end): float
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('type', 'asset')
    ...
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('type', 'liability')
            ->where('sub_type', 'current')
            ->sum('balance');
    }
    ...
    {
        return \App\Modules\Finance\Models\BankTransaction::whereBetween('transaction_date', [$start, $end])
            ->where('type', 'deposit')
            ->sum('amount');
    }
    ...
    {
        return \App\Modules\Finance\Models\Expense::whereBetween('expense_date', [$start, $end])
    ...
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('code', 'like', '15%')->sum('balance');
    }

    private function getReceivables($start, $end): float
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('code', 'like', '11%')->sum('balance');
    }

    private function getPayables($start, $end): float
    {
        return \App\Modules\Finance\Models\ChartOfAccount::where('code', 'like', '21%')->sum('balance');
    }
    ...
    private function getChangesInWorkingCapital($start, $end): array
    {
        return [
            ...
            ['item' => 'Change in \App\Modules\Inventory\Models\Inventory', 'amount' => 3000],
