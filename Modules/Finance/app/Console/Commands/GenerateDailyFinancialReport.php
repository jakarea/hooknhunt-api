...
use App\Modules\Finance\Models\DailyFinancialReport;
...
class GenerateDailyFinancialReport extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $totalRevenue = \App\Modules\Finance\Models\JournalItem::whereHas('account', function($q) {
                    $q->where('type', 'income');
                })
            ...
            $totalExpenses = \App\Modules\Finance\Models\JournalItem::whereHas('account', function($q) {
            ...
            // Get bank balances
            $cashBalance = \App\Modules\Finance\Models\Bank::where('type', 'cash')->where('status', 'active')->sum('current_balance');
            $bankBalance = \App\Modules\Finance\Models\Bank::where('type', 'bank')->where('status', 'active')->sum('current_balance');
            $mobileWalletBalance = \App\Modules\Finance\Models\Bank::whereIn('type', ['bkash', 'nagad', 'rocket'])
                                        ->where('status', 'active')
                                        ->sum('current_balance');

            // Calculate total assets (all asset accounts)
            $totalAssets = \App\Modules\Finance\Models\JournalItem::whereHas('account', function($q) {
                    $q->where('type', 'asset');
                })
                ->whereHas('journalEntry', function($q) use ($date) {
            ...
            $totalLiabilities = \App\Modules\Finance\Models\JournalItem::whereHas('account', function($q) {
            ...
            // Calculate equity
            $equity = \App\Modules\Finance\Models\JournalItem::whereHas('account', function($q) {
                    $q->where('type', 'equity');
                })
                ->whereHas('journalEntry', function($q) use ($date) {
                    $q->where('journal_entries.date', '<=', $date->format('Y-m-d'));
            ...
            $transactionsCount = \App\Modules\Finance\Models\JournalEntry::whereBetween('date', [$date->copy()->startOfDay(), $date->copy()->endOfDay()])->count();
            ...
            $this->table(
                ...
                [
                    ...
                    ['\App\Modules\Finance\Models\Bank Balance', number_format($bankBalance, 2)],
    ...
    {
        return \App\Modules\Finance\Models\Bank::active()
