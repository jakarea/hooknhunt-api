...
class AuditLogMiddleware
{
    ...
    private function logGeneralAction($request, $response)
    {
        ...
        if (!File::exists($filePath)) {
            $header = "Date,\App\Modules\System\Models\User ID,\App\Modules\System\Models\User Name,Method,URL,IP,Payload,Status Code" . PHP_EOL;
    ...
    private function logAuthAction($request, $response)
    {
        ...
        if (!File::exists($filePath)) {
            $header = "Date,Action,Identity,IP,Status Code,\App\Modules\System\Models\User Agent" . PHP_EOL;
            File::put($filePath, $header);
        }
        ...
        $data = [
            ...
            'ip' => $request->ip(),
            'status_code' => $response->getStatusCode(),
            'user_agent' => $request->header('\App\Modules\System\Models\User-Agent'),
        ];
