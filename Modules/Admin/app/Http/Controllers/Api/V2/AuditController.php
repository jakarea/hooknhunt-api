...
class AuditController extends Controller
{
    ...
    public function downloadDocument(int $id): JsonResponse
    {
        ...
        $document = DB::table('document_attachments')->where('id', $id)->first();
        ...
        // Basic access check - user must be admin or have access to the entity
        $hasAccess = auth()->user()->hasRole('admin') ||
    ...
    public function entityDocuments(Request $request): JsonResponse
    {
        ...
        // Use direct database access for module independence
        $documents = DB::table('document_attachments as da')
            ->leftJoin('users as u', 'da.uploaded_by', '=', 'u.id')
            ->leftJoin('audit_logs as al', 'da.entity_type', '=', DB::raw('CONCAT(?, ".", da.entity_id)'))
            ->where('da.entity_type', $validated['entity_type'])
            ->where('da.entity_id', $validated['entity_id'])
            ->select('da.*', 'u.name as uploader_name')
