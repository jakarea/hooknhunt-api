...
class SteadfastWebhookController
{
    ...
    private function handleDeliveryStatus(array $payload): JsonResponse
    {
        ...
        DB::transaction(function () use ($order, $newStatus, $steadfastStatus, $payload, $trackingMessage, $codAmount, $deliveryCharge, $consignmentId) {
            ...
            // Log webhook activity - pure function, direct DB access
            DB::table('website_order_activity_logs')->insert([
                'sales_order_id' => $order->id,
                'action' => 'webhook_received',
                'description' => "SteadFast webhook: {$steadfastStatus}",
                'old_values' => null,
                ...
                'performed_by' => null, // Webhook doesn't have a user
                'created_at' => now(),
                'updated_at' => now(),
            ]);
        });
    ...
    private function handleTrackingUpdate(array $payload): JsonResponse
    {
        ...
        DB::transaction(function () use ($order, $payload, $trackingMessage, $consignmentId) {
            ...
            // Log activity - pure function, direct DB access
            DB::table('website_order_activity_logs')->insert([
                'sales_order_id' => $order->id,
                'action' => 'tracking_update',
                'description' => "Tracking update: {$trackingMessage}",
                'old_values' => null,
                'new_values' => json_encode([
    ...
    /**
     * Log status change to history - pure function, direct DB access
     *
     * @param WebsiteOrder $order
    ...
    {
        DB::table('website_order_status_histories')->insert([
            'sales_order_id' => $order->id,
            'from_status' => $oldStatus,
            'to_status' => $newStatus,
            'note' => $note . ' (via SteadFast webhook)',
            'changed_by' => null, // No user for webhooks
            'created_at' => now(),
            'updated_at' => now(),
        ]);
    }
