...
class OrderStatusTransitionService
{
     ...
     *
     * Business Flow:
     * Pending → On Hold → Approved → Processing → Sent to SteadFast → In \App\Modules\Website\Models\Review → In Transit
     *                                                                              ↓
     *                                                                        Partial Delivered → Delivered → Delivered \App\Modules\Finance\Models\Payment \App\Modules\Website\Models\Review → Completed
     *                                                                              ↓
    ...
    private const VALID_TRANSITIONS = [
        ...
        'sent_to_steadfast' => [
            'in_review' => ['requires_note' => false, 'label' => 'In \App\Modules\Website\Models\Review'],
            'cancelled' => ['requires_note' => true, 'requires_cancellation_reason' => true, 'label' => 'Cancelled'],
        ],
        'in_review' => [
        ...
        'partial_delivered' => [
            ...
            'delivered_payment_review' => ['requires_note' => true, 'label' => 'Delivered – \App\Modules\Finance\Models\Payment \App\Modules\Website\Models\Review'],
        ],
        'delivered' => [
            'completed' => ['requires_note' => false, 'label' => 'Completed'],
            'delivered_payment_review' => ['requires_note' => true, 'label' => 'Delivered – \App\Modules\Finance\Models\Payment \App\Modules\Website\Models\Review'],
     ...
     * - in_transit → in_transit (\App\Modules\Shipping\Models\Shipment on the way)
    ...
    public function formatNote(string $note, ?string $cancellationReason = null): string
    {
        if ($cancellationReason) {
            $reasonLabels = [
                'customer' => 'Cancelled by \App\Modules\CRM\Models\Customer',
