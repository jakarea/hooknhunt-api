...
class OrderManagementService
{
    ...
    public function addItem(int $orderId, array $itemData): array
    {
        ...
        // Get variant data using direct database access for module independence
        $variant = DB::table('product_variants')->where('id', $itemData['product_variant_id'])->first();

        if (!$variant) {
            return ['success' => false, 'message' => 'Variant not found', 'code' => 404];
        }
    ...
    public function updateItem(int $orderId, int $itemId, array $itemData): array
    {
        ...
        return DB::transaction(function () use ($order, $item, $itemData) {
            ...
            if (isset($itemData['product_variant_id'])) {
                // Get variant data using direct database access for module independence
                $newVariant = DB::table('product_variants')->where('id', $itemData['product_variant_id'])->first();
                if ($newVariant) {
    ...
    public function removeItem(int $orderId, int $itemId): array
    {
        ...
        return DB::transaction(function () use ($order, $item) {
            // Module independence: Use denormalized product_name instead of variant relationship
            $variantName = $item->product_name ?? "Item #{$item->id}";
