...
class SSLCommerzService
{
    ...
    public function verifyIPN(array $data): array
    {
        ...
        // Find payment transaction using direct DB access (pure function - no cross-module dependencies)
        $payment = DB::table('payment_transactions')
            ->where('gateway_tran_id', $data['tran_id'])
        ...
        if ($status === 'SUCCESS' || $status === 'VALID') {
            ...
            // Update sales order
            $salesOrder = DB::table('sales_orders')->where('id', $payment->sales_order_id)->first();
            if ($salesOrder) {
        ...
        if ($status === 'FAILED') {
            ...
            DB::table('payment_transactions')
                ...
                ->update([
                    'status' => 'failed',
                    'failed_reason' => $data['error_reason'] ?? 'Payment failed by gateway',
                    'gateway_response' => json_encode($data),
                    'updated_at' => now(),
                ]);
